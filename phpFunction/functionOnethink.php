...
function is_login(){
    if (isAppRequest()) {
        //APP下直接获取http header 下的uid
        if (empty($_SERVER['HTTP_X_FW_TOKEN'])) {
            return null;exit();
        }else{
            // 800:MTQ0NDIwMzY4M4F3dqs:1444203683000 这是token数据格式 userid：token：过期时间
            $trueToken = explode(':',$_SERVER['HTTP_X_FW_TOKEN']);
            $resultPassword = asyncRequest("{$_SERVER['REQUEST_SCHEME']}://{$_SERVER['SERVER_NAME']}".U('user/getPasswordByUid'),array('uid'=>$trueToken[0]));
            $oldToken = md5($trueToken[0].$trueToken[2].$resultPassword);
            
            if ($oldToken == $trueToken[1]) {
                $phpTime = substr($trueToken[2],0,-3);
                if ($phpTime < time()) {
                    return null;exit();
                }
                return $trueToken[0];
...
function loginService(){
    ...
    // if (empty($result) or !is_numeric($result) or $result == 0) {
    //     if (isAppRequest()) {
    //         return $this->error('您还没有登录，请先登录！',401);
    //     }else{
...
function isAppRequest(){
    // if(!$this->authenticate($_SERVER['HTTP_X_FW_PASSWORD'])){
    //     // return print json_encode(array('status'=>0,'info'=>'aa密码错误'));
    //     return false;
    // }
    //密码
    $prev3str = substr($_SERVER['PATH_INFO'],0,3);
    
    if (!empty($_SERVER['PATH_INFO']) and $prev3str == 'api') {
        return true;
    }else{
        return false;
    }
 ...
 * @author huajie <banhuajie@163.com>
 */
function get_cover($cover_id, $field = null,$isRmDomain =false){
    if(empty($cover_id)){
        //因为数据库数据不全，暂时只能这样
        if (isAppRequest()) {
            return null;
    ...
    if($field == 'path'){
        ...
        if(!empty($picture['url'])){
            ...
            if (isAppRequest() and $isRmDomain == false && !empty($picture)) {
                $picture['path'] ="{$_SERVER['REQUEST_SCHEME']}://{$_SERVER['SERVER_NAME']}". __ROOT__.$picture['path'];
            }elseif ($isRmDomain == true) {
...
function get_memberface($id= -1) {
    ...
    if (isAppRequest()) {
        return "{$_SERVER['REQUEST_SCHEME']}://{$_SERVER['SERVER_NAME']}".$photo_url;
    }
    return $photo_url;
}

function memberupdate($id,$photo_url) {
    $list = S('sys_active_member_list');
    $key = "u{$id}";
    if (isset($list[$key])) {
        $list[$key]['photo_url'] = $photo_url;
        S('sys_active_member_list', $list);
    }
}
 ...
 * @return string           转译列表
 * @author 廖夏明 <liaoxiaming@crowdfunding.com>
 */
...
* @param   $para  传递参数 
**/
function getHttpResponseGet($url,$para=null) {
    $ch = curl_init();
...
* @param   $para  传递参数 
**/
function getHttpResponseHttps($url,$para=null) {
    $ch = curl_init();
